...
import type { AdminSimple } from '@/pg-ui/service/api';
...
export default function AdminsSelector<T extends FieldValues>({ control, name, onAdminsChange, disabled = false }: AdminSelectorProps<T>) {
  ...
  const allAdmins = (adminsData?.admins || []) as AdminSimple[]
  const filteredAdmins = allAdmins.filter(admin => admin.username.toLowerCase().includes(searchQuery.toLowerCase()))
