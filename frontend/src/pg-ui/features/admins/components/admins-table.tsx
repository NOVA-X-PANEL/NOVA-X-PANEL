...
import type { AdminDetails, AdminsListResponse } from '@/pg-ui/service/api'
...
export default function AdminsTable({ onEdit, onDelete, onToggleStatus, onResetUsage, onTotalAdminsChange }: AdminsTableProps) {
  ...
  const adminsResponseData = adminsResponse as AdminsListResponse | undefined
  const adminsData: AdminDetails[] = adminsResponseData?.admins || []
  ...
  useEffect(() => {
    if (onTotalAdminsChange) {
      if (adminsResponseData) {
        onTotalAdminsChange({
          total: adminsResponseData.total,
          active: adminsResponseData.active,
          disabled: adminsResponseData.disabled,
          limited: adminsResponseData.limited,
        })
      } else {
  ...
  return (
    <div>
      ...
      <PaginationControls
        ...
        totalPages={Math.ceil((adminsResponseData?.total || 0) / itemsPerPage)}
        itemsPerPage={itemsPerPage}
        totalItems={adminsResponseData?.total || 0}
