...
func TestServePWAAssets(t *testing.T) {
	oldDistFS := distFS
	distFS = fstest.MapFS{
		"dist/manifest.webmanifest": &fstest.MapFile{Data: []byte(`{"name":"nova-x-panel"}`)},
		"dist/pwa-register.js":      &fstest.MapFile{Data: []byte("register")},
		"dist/service-worker.js":    &fstest.MapFile{Data: []byte("worker")},
		"dist/icons/nova-x-panel-192.png":  &fstest.MapFile{Data: []byte("icon-192")},
		"dist/icons/nova-x-panel-512.png":  &fstest.MapFile{Data: []byte("icon-512")},
	}
	...
	tests := []struct {
		...
		{name: "manifest", handler: ServePWAManifest, contentType: "application/manifest+json; charset=utf-8", body: `{"name":"nova-x-panel"}`},
...
func TestServePWAIconServesPNG(t *testing.T) {
	...
	response := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(response)
	context.Params = gin.Params{{Key: "name", Value: "nova-x-panel-192.png"}}
