...
type GroupSimple = { id: number; name: string }
...
export default function GroupsSelector<T extends FieldValues>({ control, name, onGroupsChange, disabled = false }: GroupsSelectorProps<T>) {
  ...
  const allGroups = (groupsData?.groups || []) as GroupSimple[]
  const filteredGroups = allGroups.filter(group => group.name.toLowerCase().includes(searchQuery.toLowerCase()))
