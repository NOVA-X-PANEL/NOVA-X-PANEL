...
Thanks for taking the time to contribute to nova-x-panel. This guide gets a development panel running locally and explains the conventions the project follows so changes land cleanly.
...
```bash
git clone https://github.com/NOVA-X-PANEL/nova-x-panel.git
cd nova-x-panel
...
The repo checks in two VS Code launch profiles in `.vscode/launch.json`: **Run nova-x-panel (Debug)** for the default SQLite setup, and **Run nova-x-panel (Postgres)** which points `XUI_DB_TYPE`/`XUI_DB_DSN` at a local PostgreSQL. The Postgres profile also prepends the PostgreSQL `bin` to `PATH` so the panel can find `pg_dump`/`pg_restore` (the `postgresql-client` tools used for DB backup/restore) — adjust the DSN and that path to your machine:
...
```jsonc
{
  ...
  "configurations": [
    ...
    {
      "name": "Run nova-x-panel (Postgres)",
      "type": "go",
      "request": "launch",
      "mode": "auto",
      "program": "${workspaceFolder}",
...
- Bug reports and feature requests: [GitHub Issues](https://github.com/NOVA-X-PANEL/nova-x-panel/issues)
