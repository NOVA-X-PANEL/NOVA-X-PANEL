...
# ---------------------------------------------------------------------------
# banner — the one thing this installer does differently from upstream
# ---------------------------------------------------------------------------
banner_unicode() {
    cat <<'NOVA_ART'
███╗   ██╗  ██████╗  ██╗   ██╗  █████╗      ██╗  ██╗
...
# Fallback for a terminal that is not UTF-8: block glyphs would be mojibake.
banner_ascii() {
    cat <<'NOVA_ART'
============================================================
                     N O V A   X
...
# Bright neon blue where the terminal supports it, bright blue otherwise.
neon_colour() {
    local n
...
print_banner() {
    # XUI_NO_BANNER=1 keeps the output script-friendly.
    ...
    echo
    printf '%b%s%b\n' "$colour" "$body" "$reset"
    printf '%b  NOVA X PANEL  ·  built on 3x-ui (GPLv3)%b\n' "$colour" "$reset"
    printf '%b  https://github.com/NOVA-X-PANEL/NOVA-X-PANEL%b\n\n' "$colour" "$reset"
}
...
resolve_latest_tag() {
    local url tag
    url=$(curl -sSLI -o /dev/null -w '%{url_effective}' --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://github.com/NOVA-X-PANEL/NOVA-X-PANEL/releases/latest" 2>/dev/null)
    tag=${url##*/tag/}
    if [[ "$tag" != "$url" && -n "$tag" && "$tag" != "latest" ]]; then
        echo "$tag"
        return 0
    fi
    curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/NOVA-X-PANEL/NOVA-X-PANEL/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/'
}
...
require_repo_files() {
    ...
    [[ "${ref}" == "main" ]] && return 0
    for name in "$@"; do
        status=$(curl -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --speed-limit 1 --speed-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/NOVA-X-PANEL/NOVA-X-PANEL/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        verify_release_checksum "https://github.com/NOVA-X-PANEL/NOVA-X-PANEL/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz" "${xui_folder}-linux-$(arch).tar.gz"
    else
        tag_version=$1
        ...
        url="https://github.com/NOVA-X-PANEL/NOVA-X-PANEL/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.sh"
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.rc"
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.service.rhel"
                    ;;
            esac
