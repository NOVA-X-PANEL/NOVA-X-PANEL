...
require_repo_files() {
    ...
    [[ "${ref}" == "main" ]] && return 0
    for name in "$@"; do
        status=$(${curl_bin} -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
update_x-ui() {
    ...
    if [[ -n "${XUI_UPDATE_TAG}" ]]; then
        ...
        tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/NOVA-X-PANEL/NOVA-X-PANEL/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/NOVA-X-PANEL/NOVA-X-PANEL/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2> /dev/null
    if [[ $? -ne 0 ]]; then
    ...
    ${curl_bin} -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.sh" > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        ${curl_bin} -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/NOVA-X-PANEL/NOVA-X-PANEL/${script_ref}/x-ui.rc" > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
