...
#   * /etc/nova-x-panel/install-result.env exists (mode 600) with random, non-default creds
...
docker run --rm \
    ...
    "$IMAGE" bash -euo pipefail -c '
        ...
        /usr/local/nova-x-panel/x-ui setting -show | grep -q "hasDefaultCredential: false" \
        ...
        echo "custom-sentinel" > /usr/local/nova-x-panel/bin/geoip_custom.dat
        geoip_sum_before=$(sha256sum /usr/local/nova-x-panel/bin/geoip.dat | cut -d" " -f1)
        ...
        test -f /usr/local/nova-x-panel/bin/geoip_custom.dat \
            || { echo "FAIL: custom bin/ file did not survive a second install"; exit 1; }
        [ "$(cat /usr/local/nova-x-panel/bin/geoip_custom.dat)" = "custom-sentinel" ] \
            || { echo "FAIL: custom bin/ file content changed across a second install"; exit 1; }
        geoip_sum_after=$(sha256sum /usr/local/nova-x-panel/bin/geoip.dat | cut -d" " -f1)
