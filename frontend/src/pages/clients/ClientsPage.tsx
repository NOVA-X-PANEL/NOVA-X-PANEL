...
import { ClientsActions, ClientsHero } from './ClientsChrome';
...
// The reference's stat card: a neon icon tile, a label, a big value. Clicking it
// filters the table, so the tile is the button.
function SummaryStat({ title, value, icon, tint, emails, selected, onSelect }: SummaryStatProps) {
  const stat = (
    <div
      role="button"
      tabIndex={0}
      aria-pressed={selected}
      className={`nc-stat nc-stat-filter${selected ? ' is-selected' : ''}`}
      onClick={onSelect}
      onKeyDown={activateOnKey(onSelect)}
    >
      <span className={`nc-stat-icon ${tint}`}>{icon}</span>
      <span className="nc-stat-label">{title}</span>
      <span className="nc-stat-value">{value}</span>
    </div>
  );
            ...
            <Spin spinning={!fetched} delay={200} description={t('loading')} size="large">
              {!fetched ? (
                ...
                <div className="nc-page">
                  ...
                  <div className="nc-stats nc-stats-6">
                    <SummaryStat
                      title={t('clients')}
                      value={summary.total}
                      icon={<TeamOutlined />}
                      ...
                      >
                          ...
                          <div className="filter-chips">
                            ...
                            {filters.autoRenew && (
                              <Tag
                                closable
                                color="gold"
                                onClose={() => clearOneFilter('autoRenew')}
                              >
                        ...
                        {!isMobile ? (
                          <Table<ClientRecord>
                            ...
                            rowKey="email"
                            rowClassName={() => 'nc-row-accent'}
                            onRow={(record) => ({
                              style: {
                                ['--row-accent' as string]: protocolHue(
