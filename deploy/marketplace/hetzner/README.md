# nova-x-panel on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship nova-x-panel via **cloud-init**: each instance installs non-interactively and
...
nova-x-panel non-interactively and generates unique per-instance credentials.
...
is essentially a documented cloud-init config plus metadata. To propose nova-x-panel as
