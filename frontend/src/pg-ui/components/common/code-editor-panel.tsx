...
type FocusableCodeEditor = { focus?: () => void }
  ...
  const handleEditorDidMount = useCallback(
    (editor: unknown) => {
      ...
      if (!isMobile) {
        ...
        const monacoEditor = editor as { onDidBlurEditorWidget?: (callback: () => void) => { dispose: () => void } }
        if (onDidBlur && monacoEditor.onDidBlurEditorWidget) {
          blurDisposableRef.current = monacoEditor.onDidBlurEditorWidget(() => onDidBlur())
        }
    ...
    return (
      <Suspense fallback={editorFallback}>
        <MonacoEditor
          ...
          options={monacoOptionsMerged as never}
        />
      </Suspense>
    )
  }
      ...
      >
        <DialogContent
          ...
          onOpenAutoFocus={e => {
            ...
            setTimeout(() => {
              if (editorInstance) {
                const focusableEditor = editorInstance as FocusableCodeEditor
