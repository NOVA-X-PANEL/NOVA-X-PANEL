# nova-x-panel — Architecture & Code Map
...
> tree rather than a pinned release (Go module `github.com/nova-x-panel/nova-x-panel/v3`).
>
...
nova-x-panel is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
- **`mtg-multi` for MTProto inbounds** (`github.com/nova-x-panel/mtg-multi`, a multi-secret fork
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other nova-x-panel
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("nova-x-panel")
...
A "node" (`model.Node`) is another nova-x-panel instance this panel controls. Every state-changing
...
- **Module path is `.../v3`.** Internal imports use `github.com/nova-x-panel/nova-x-panel/v3/internal/...`.
