...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/NOVA-X-PANEL/nova-x-panel/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/NOVA-X-PANEL/nova-x-panel/main/update.sh)
    if [[ $? == 0 ]]; then
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/NOVA-X-PANEL/nova-x-panel/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/NOVA-X-PANEL/nova-x-panel/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/NOVA-X-PANEL/nova-x-panel/main/x-ui.sh"
    fi
}
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/nova-x-panel/nova-x-panel/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/nova-x-panel/nova-x-panel/master/install.sh)${plain}"
    echo ""
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│  ${green}NOVA-X-PANEL Panel Management Script${plain}                │
